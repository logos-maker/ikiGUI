...
void main(){
	...
	ikigui_map_init(&menubar,&mywin.frame,&pic,OFFSET_ASCII,0,0,8,8,48,1); // init the setings for the tilemap used as text area
	...
	while(1) { // Event loop! look for events forever...
		...
		if(mywin.mouse.buttons & MOUSE_LEFT){ ikigui_blit_fast(&mywin.frame,&pic,mywin.mouse.x,mywin.mouse.y,&rect); } // blit image part to window if mouse down
