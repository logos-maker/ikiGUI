...
void main(){
	...
	ikigui_window_open(&mywin, "Line & Circle", 1024, 800); // Create and open window and specify window size
	ikigui_image_solid(&mywin.image,0xFF333333);
	...
	while(1) { // Event loop! look for events forever...
		...
		ikigui_draw_circle_parts(&mywin.image,NE|SW,0xffff0000,500,400,mywin.mouse.x>>2);
		ikigui_draw_line(&mywin.image,0xffffffff,mywin.mouse.x,mywin.mouse.y,0,0);
                sprintf((char*)menubar.map,"%04d %04d",mywin.mouse.x,mywin.mouse.y); // Generate text bytes
