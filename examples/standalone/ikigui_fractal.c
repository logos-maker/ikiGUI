...
// gcc ikigui_standalone.c -o standalone -lX11 -lm
// for windows... (-mwindows gets rid of terminal opening)
// gcc ikigui_standalone.c -o standalone.exe -lgdi32 -mwindows -lole32 -lm
#define IKIGUI_DIAL
#define IKIGUI_IMAGE
...
void main(){
	...
	#define SCREEN_H 800
	ikigui_window_open(&mywin,"Hello World!",1024,800); // Create and open window and specify window size
...
*/
	...
	if(fractal != NULL) ikigui_save_png(&mywin.image, fractal);
	...
	while(1) { // Event loop! look for events forever...
		...
		if(mywin.mouse.buttons & MOUSE_LEFT){ ikigui_tile_fast(&mywin.image,&pic,mywin.mouse.x,mywin.mouse.y,NULL); } // blit image part to window if mouse down
                ...
                sprintf((char*)menubar.map, "X: %04d  Y: %04d", mywin.mouse.x, mywin.mouse.y);// Generate text bytes
