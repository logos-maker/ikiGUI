...
void ikigui_image_create(ikigui_image *frame, uint32_t w,uint32_t h){
        frame->w = w;
        frame->h = h;
...
void ikigui_blit_hollow(ikigui_image *dest,ikigui_image *frame, int x, int y, ikigui_rect *part){ // Draw area - can be used if you whant to fill high alpha value with a solid color.
        if((x<0) || (y<0))return; // sheilding crash
        if(dest->w < (x+part->w))return; // shielding crash
        if(dest->h < (y+part->h))return; // shielding crash
        ...
        for(int j = 0 ; j < part->h ; j++){ // vertical
			...
			if(!dest->composit){
				dest->pixels[(x+i+(hflip(dest->h,j+y))*dest->w)] 
				= alpha_channel(dest->pixels[(x+i+(hflip(dest->h,j+y))*dest->w)], (frame->bg_color&0x00FFFFFF) | (0xFF000000 & frame->pixels[i+part->x+frame->w*(j+part->y)]));
			}else{
				dest->pixels[x+i+(j+y)*dest->w] 
				= alpha_channel(dest->pixels[x+i+(j+y)*dest->w], (frame->bg_color&0x00FFFFFF) | (0xFF000000 & frame->pixels[i+part->x+frame->w*(j+part->y)]));
			}
                }
        }
}
...
void ikigui_image_solid(ikigui_image *mywin, unsigned int color){ // Fill window.
        for(int i = 0 ; i < mywin->w * mywin->h ; i++){ // All pixels      
                        mywin->pixels[i] = color;
        }
}
void ikigui_image_gradient(ikigui_image *mywin, uint32_t color_top, uint32_t color_bot){ // Fill window.
